...
const GenerateId = () => {
  ...
  const [edit, setEdit] = useState(false);  const [query, setQuery] = useState("");
  ...
  const filterData = !query
    ...
    : users.filter(
        (i) =>
          i?.name?.toLowerCase().includes(query?.toLowerCase())  || i?.phone?.toLowerCase().includes(query?.toLowerCase())
      );
