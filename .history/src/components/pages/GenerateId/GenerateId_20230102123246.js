...
const GenerateId = () => {
  ...
  const fetchData = useCallback(async () => {
      try{
          const {data} = await axios.get('http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/api/heros/getherorole4')
          setData(data)
      }catch(err){
        console.log(err)
      }
  ...
  const filterData = !query
    ...
    : data?.services?.filter(
      ...
      const deleteHandler = async(id) => {
        try{
          const { data } = await axios.delete(`http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/api/heros/deleteheroByIdinadmin/${id}`)
          toast.success('')
        }
      }
  ...
  return (
    <>
      ...
      >
        ...
        <div
         style={{ overflow: "auto" , marginTop : '2%'  }} 
          className="response"
        >
